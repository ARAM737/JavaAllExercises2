...
import all.exercise23.Controllers.GameController;
import all.exercise23.Controllers.LevelController;
import all.exercise23.Interfaces.GameRepository;
import all.exercise23.Interfaces.LevelRepository;
import all.exercise23.ScheduleMXBean;
...
public class Schedule implements ScheduleMXBean {
    ...
    public void doScheduledTask() throws IOException {
        ...
        for (int i=0; i< games.size();i++) {
            File game = new File("C:\\Users\\AramK\\IdeaProjects\\JavaAllExercises2\\src\\main\\java\\game"+i+".txt");
            FileWriter writer = new FileWriter(game, false);
            writer.write(games.get(i).toString());
        }

        for (int i=0; i< levels.size();i++) {
            File level = new File("C:\\Users\\AramK\\IdeaProjects\\JavaAllExercises2\\src\\main\\java\\level"+i+".txt");
